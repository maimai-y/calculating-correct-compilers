...
data Code : List STy → List STy → Set where
  PUSH : (n : ℕ) → Code (typ nat ∷ S) S' → Code S S'
  ADD : Code (typ nat ∷ S) S' → Code (typ nat ∷ typ nat ∷ S) S'
  LOOKUP : var α E → Code (typ α ∷ S) S' → Code S S'
  ABS : Code [] (typ α₁ ∷ []) → Code (typ (α₂ ⇒ α₁) ∷ S) S' → Code S S'
  APP : Code (typ α₁ ∷ S) S' → Code (typ (α₂ ⇒ α₁) ∷ typ α₂ ∷ S) S'
  HALT : Code S S

comp : Expr α E → Code (typ α ∷ S) S' → Code S S'
...
compile : Expr α E → Code S (typ α ∷ S)
compile e = comp e HALT

head : Stack (typ α₁ ∷ S) → El α₁
head (x ▷ _) = x

exec : Code S S' → Stack S × Env E → Stack S' × Env E'
